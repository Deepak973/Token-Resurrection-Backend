...
router.get('/transactions', async (req: Request, res: Response) => {
    ...
    try {
        if (!token || !to) {
            return res.status(400).json({ error: 'Token and to address are required' });
        }

        const result = await getTransactionsByTokenAndTo(token as string, to as string);

        if (result.transactions.length === 0) {
            return res.status(404).json({ message: 'No transactions found for the given token and to address' });
        }
...
router.post('/transactions', async (req: Request, res: Response) => {
    ...
    try {
        ...
        res.status(500).json({ error: 'An error occurred while creating the transactions' });
    }
});
...
router.get('/user-transactions', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (transactions.length === 0) {
            return res.status(404).json({ message: 'No transactions found for the given user address' });
        }
